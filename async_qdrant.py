...
async def create_collection(collection_name: str) -> bool:
    ...
    try:
        # Attempt to get collection info; raises if not exists
...
async def delete_collection(collection_name: str) -> bool:
    ...
    try:
        # Attempt to delete; works even if collection absent
...
async def _upsert_batch(collection_name: str, batch_docs) -> None:
    ...
    embeddings = embedder.encode([doc.page_content for doc in batch_docs], show_progress_bar=False)
    points = [
        models.PointStruct(
            id=str(uuid.uuid4()),
            vector=emb.tolist(),
...
async def insert_data_parallel(docs, collection_name: str) -> bool:
    ...
    for idx in range(total_batches):
        ...
        async def worker(batch_docs, batch_idx):
        ...
        tasks.append(worker(batch, batch_idx))
